...
const top5Albums = () => {
    ...
    useEffect(() => {
        ...
        const albumCounts = d3.rollups(
            data,
            v => ({ count: v.length, artist: v[0]?.master_metadata_album_artist_name || 'Unknown' }),
            d => d.master_metadata_album_album_name
        )
            .map(d => ({ album: d[0], count: d[1].count, artist: d[1].artist }))
        ...
        const tooltip = d3.select("body")
            .selectAll(".top5-albums-tooltip")
            .data([null])
            .join("div")
            .attr("class", "tooltip top5-albums-tooltip")
        ...
        const showTooltip = (event, d) => {
            tooltip
                ...
                .style("opacity", 1)
                .html(`
                    <strong>Album: ${d.album}</strong><br/>
        ...
        const yScale = d3
            .scaleLinear()
            .domain([0, d3.max(albumCounts, (d) => d.count)])
            .range([height, 0]);
        const chart = svg
